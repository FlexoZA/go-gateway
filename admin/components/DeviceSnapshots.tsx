...
// DeviceSnapshots is the "Snapshots" tab of a device's detail page: capture a new
// still on demand, and search/view stills already stored on the device's SD card.
// Live capture goes through 0x4020 + file-transfer 0x4090 (image/jpeg inline);
// the saved-snapshot list is a file query (0x4060, ft=3/4) and each result is
// pulled by path over the same file-transfer route.
export function DeviceSnapshots({ serial, sleeping }: { serial: string; sleeping: boolean }) {
  ...
  function showPreview(p: Preview) {
    // Drop a previous blob URL when replacing it.
    if (blobRef.current) {
  ...
  return (
    <div className="space-y-8">
      <section>
        ...
        <CapturePanel serial={serial} sleeping={sleeping} onCaptured={showPreview} />
      </section>
      ...
      <div className="card space-y-3">
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <select className="input mt-1 w-32" value={camera} onChange={(e) => setCamera(Number(e.target.value))} disabled={capturing}>
              {[0, 1, 2, 3].map((c) => (
          ...
          <div>
            ...
            <select className="input mt-1 w-40" value={resolution} onChange={(e) => setResolution(Number(e.target.value))} disabled={capturing}>
              {RESOLUTIONS.map((r) => (
        ...
        <p className="text-xs text-slate-500">
          The device takes the photo and uploads it over its media link, so a capture can take a few seconds.
        </p>
      </div>
    </div>
  );
                ...
                <tr key={s.device_path}>
                  ...
                  <td className="td">
                    <div className="flex justify-end gap-2">
                      ...
                      <a
                        className="btn-ghost"
                        href={`/api/gw/units/${encodeURIComponent(serial)}/snapshots/file?path=${encodeURIComponent(s.device_path)}`}
                        download={baseName(s.device_path)}
                      >
                        Download
                      </a>
                    </div>
