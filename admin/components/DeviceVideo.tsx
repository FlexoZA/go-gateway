...
// DeviceVideo is the "Video" tab of a device's detail page: a live stream and a
// per-device clip workflow (search recordings → pull → download). It reuses the
// same gateway endpoints as the standalone Clips/Live pages but scopes everything
// to one device, so there's no device picker. Those standalone pages are left
// as-is for the fleet-wide view.
  ...
  async function findRecordings() {
    ...
    try {
      const res = await api<{ recordings: Recording[]; count: number }>(
        `units/${encodeURIComponent(serial)}/recordings?camera=${camera}&profile=${profile}&start_utc=${startUtc}&end_utc=${endUtc}`,
      );
