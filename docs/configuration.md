...
| `ERROR_LOG_RETENTION_DAYS` | `30` | **Seeds** the `error_log_retention_days` server setting on first run: how long `gateway_errors`/`device_errors` rows are kept before an hourly reaper deletes them. `0` = keep forever. Thereafter edit it live in the admin (Server Settings → Error log retention); this env value only sets the initial default |
